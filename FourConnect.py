...
class FourConnect:
    ...
    def _CheckHorizontal(self,row,col,player):
        ...
        #go till the leftmost point where you've seen only this player
        while True:                     
            cMin = cMin-1
        ...
        #go till the rightmost point where you've seen only this player
        while True:                    
            cMax = cMax+1
        ...
        #if there's a horizontal with length >= 4 of the same player, he wins
        if cMax-cMin>=3:
            return True
        else:
            return False
    ...
    def _FindMyopicMoves(self):
        ...
        for action in range(7):
            ...
            if row==-1:
                ...
                #check if GameTreePlayer can win in the next move
                if row>0:
            ...
            #check if this action leads to Myopic Player directly winning 
            win=self._CanMyopicPlayerWin(row,action)
            if win==True:
                myopicWinAction=action
                
            #see if this (row,col) needs to be blocked to prevent GameTreePlayer from winning 
    ...
    def _FindBestMyopicAction(self):
        ...
        # if MyopicPlayer can win in this move, choose this and end game
        if myopicWinAction != None:
            bestAction = myopicWinAction
        
        # if GameTreePlayer can win in next move, choose this and block
        elif gameTreeWinAction != None:
            bestAction = gameTreeWinAction
            
        # else choose any valid action randomly, & if none, choose losing action 
